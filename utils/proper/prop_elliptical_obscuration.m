function bm = prop_elliptical_obscuration(bm, rx, ry, cx, cy, norm)
%        bm = prop_elliptical_obscuration(bm, rx, ry, cx, cy, norm)
% Multiplies the wavefront in bm by an elliptical obscuration
% bm   = beam structure (input and output)
% rx   = radius of obscuration x (meters unless norm = 1)
% ry   = radius of obscuration y (meters unless norm = 1)
...
% 2016 Apr 14  gmg  Matlab translation
  ...
  bm.wf = bm.wf.*prop_shift_center(prop_ellipse(bm,rx,ry,cx,cy,1.0,norm),1);
