function bm = prop_circular_obscuration(bm, ra, cx, cy, norm)
% [bm] = prop_circular_obscuration(bm, ra, cx, cy, norm)
% Multiplies the wavefront in bm by a circular obscuration
% bm   = beam structure (input and output)
% ra   = radius of obscuration (meters unless norm = 1)
...
% 2014 Sep 02  gmg  Changed prop_shift_center call to allow odd size arrays
  ...
  bm.wf = bm.wf.*prop_shift_center(prop_ellipse(bm,ra,ra,cx,cy,1.0,norm),1);
