function [ G ] = calculateGmatrix_vFiberCouplingOneFibxel( bmb4dm, surf_DM1_init, Q, Nact, ac_spac, poke_amp, infl, lambda0, N , info)
...
%             end
            if(normal_EFC)
                G(:,count) = FP_col*factor_fiber_thruput-FP0_col*factor_fiber_thruput; % The column of the G matrix is the difference in the field 
            else
                if(use_fiber)
                    G(1,count) = sum(sum(FP_col.*fibermode0(:)))-sum(sum(FP0_col.*fibermode0(:))); % The column of the G matrix is the difference in the field 
                else
