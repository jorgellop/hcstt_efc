function plot_field_Gu_Eab_test(wf,wf0, Eab_re, Eab_im, info)
...
full_path = [info.outDir,'Field',filesep,'FieldOverFibxel_Eabre',num2str(Eab_re),'_Eabim',num2str(Eab_im),'.png'];
...
subplot('Position',pos1)
imagesc([x_fib_pix-fiberDiam_pix/2+1:x_fib_pix+fiberDiam_pix/2]/lambdaOverD,[y_fib_pix-fiberDiam_pix/2+1:y_fib_pix+fiberDiam_pix/2]/lambdaOverD,real(Gu_trim),[-8e-6 8e-6]);
colorbar; 
axis image;
set(gca,'FontSize',7)
titl = ['Gu: Re(Gu) over Fibxel'];
title(titl,'FontSize', 7);
...
subplot('Position',pos1)
imagesc([x_fib_pix-fiberDiam_pix/2+1:x_fib_pix+fiberDiam_pix/2]/lambdaOverD,[y_fib_pix-fiberDiam_pix/2+1:y_fib_pix+fiberDiam_pix/2]/lambdaOverD,imag(Gu_trim),[-8e-6 8e-6]);
colorbar; 
axis image;
set(gca,'FontSize',7)
titl = ['Gu: Im(Gu) over Fibxel'];
title(titl,'FontSize', 7);
xlabel('x (lam/D)','FontSize', 7);
set(gca,'YTick',[])
% ylabel('y (lam/D)','FontSize', 7);
