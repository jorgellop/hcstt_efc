...
function MultiDM_Command = NK_MultiDM_Command(map, flat)

% Reshape the map in nm as a Vector
MultiDM_Command = reshape(flipud(map),[],1);  
MultiDM_Flat = reshape(flipud(flat),[],1);  

% Convert nm in V || Equation from DM characteristics sheet. 
MultiDM_Command = - 30.16 + 5.704e-5 * sqrt(7.01264e9 * (MultiDM_Command + MultiDM_Flat) + 2.79527e11);
