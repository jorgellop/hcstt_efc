% test_EFSensing_RegularEFC_CovEstimator
%
% Test EF sensing for Regular EFC pixel-based on the bench
% Find noise covariances.
%
%
...
label = '_testEFSensing_RegularEFC_Covariance_Estimator_Aug16';
...
if(one_it)
    ...
    numtry = 1800; % Should be running till morning.
    yk = zeros(numtry,num_DM_shapes*num_Q);
    ...
    [DeltaI, H] = EFSensing_RegularEFC_labTest_Cov(wf2_current, zeros(Nact^2,1), info); % Initial guess
    ...
    for k = 1:numtry
        ...
        [DeltaI, ~] =  EFSensing_RegularEFC_labTest_Cov(wf2_current,zeros(Nact^2,1),info);
        yk(k,:) = DeltaI;
...
save('KF_Data_for_cov_estimate_0816', 'yk', 'H', 'Q_mat', 'R_mat');

Qnew = zeros('like', Q_mat);
Rnew = zeros('like', R_mat);
...
save('KF_cov_estimate_0816', 'Qnew', 'Rnew');